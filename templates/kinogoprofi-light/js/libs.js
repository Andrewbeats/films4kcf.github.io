...
$(document).ready(function(){
	...
	$("body").addClass("js");
		 ...
		 $("#owl-carou").owlCarousel({    
			...
			paginationSpeed : 400,
			singleItem:false,
			itemsCustom: [[0, 2], [470, 3], [590, 4], [760, 5], [950, 6], [1220, 7]],
			autoPlay:false,
			pagination: false,
			lazyLoad : true
		});
		 $("#owl-rel").owlCarousel({    
			...
			paginationSpeed : 400,
			singleItem:false,
			itemsCustom: [[0, 2], [470, 3], [590, 4], [760, 5], [950, 4], [1220, 5]],
			autoPlay:false,
			pagination: false,
			lazyLoad : true
		});
	...
	$('body').on('click','.pseudo-link',function(){
		...
		$('.sorting-menu > li, .change-here').click(
		function() {
			$(this).children('.sorting-list').slideToggle(200);
			$(this).siblings().children('.sorting-list').slideUp(200);
		});	
	...
	$('#top').append('<div class="show-menu"><i class="fa fa-bars"></i></div><div class="show-search"><i class="fa fa-search"></i></div>');  
    $(".show-menu").click(function(){
		$("#cols, .first-menu, .carousel-wr, .wr").toggleClass('vis-side');
		$(this).find('i').toggleClass('fa-bars fa-times');
		$('html, body').animate({
		scrollTop: $("#cols").offset().top - 50
		}, 600);
	});
	...
	if ($(window).width() < 950) {
		$('.second-menu li:not(.social-here)').each(function(){
			var $item = $(this);
			$item.appendTo('#mob-menu');
		});
	};
	if ($(window).width() < 760) {
		$('.first-menu > li:not(.change-here)').each(function(){
			var $item2 = $(this);
			$item2.appendTo('#mob-menu');
		});
	};
...
$(document).ready(function(){
		...
		if($.cookie('loop_side') == 'grid-row' ) {
		$('#grid-serial').removeClass('grid-th').addClass('grid-row').attr('data-view', 'grid-row');
		$('#grid-side span:nth-child(1)').addClass('current').siblings('span').removeClass('current');
		} 
		if($.cookie('loop_side') == 'grid-th' ) {
		$('#grid-serial').removeClass('grid-row').addClass('grid-th').attr('data-view', 'grid-th');
		$('#grid-side span:nth-child(2)').addClass('current').siblings('span').removeClass('current');
		} 

		
		$('#grid-side span').on('click', function(e) {
			e.preventDefault();
			...
			$.cookie('loop_side', viewType, { path: '/', expires : 7});
...
$(document).ready(function(){
	if ($(window).width() > 760) {
			var $urlBg = $('#kino-page').attr("data-bg");
			if ($urlBg) {
			$('#wr-bg').addClass('have-bg').prepend('<div id="bg-wallpaper"></div>');
			$('#bg-wallpaper').css({
